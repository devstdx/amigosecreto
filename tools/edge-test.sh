...
check "config de Pages presente (wrangler.toml → pages_build_output_dir)" \
  "$(grep -c 'pages_build_output_dir' "$(dirname "$0")/../wrangler.toml" 2>/dev/null || echo 0)" "1"
check "binding D1 declarado en la config de Pages" \
  "$(grep -c 'binding = \"DB\"' "$(dirname "$0")/../wrangler.toml" 2>/dev/null || echo 0)" "1"
check "el binding D1 está declarado en wrangler.jsonc (Workers)" \
