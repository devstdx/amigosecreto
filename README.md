...
**CI/CD con el dashboard (lo que ya tienes conectado)**: si conectaste el repo en
**Workers & Pages → Create → Workers → Import a repository**, el build usa estos
comandos. Se configuran en *tu Worker → Settings → Build*:
...
- **v1.4.1 — puesta en marcha de D1 en producción**:
  - `npm run db:configure -- <database_id>` (`tools/d1-configure.mjs`): fija el ID
    ...
    relleno. Antes había que editar dos archivos a mano.
  - `npm run db:create` fija la región (`--location weur`, Europa Oeste).
  - README: flujo de CI del dashboard (build = `npm run db:remote`, deploy =
    `npx wrangler deploy`) y diagnóstico del error `code: 10181`.
